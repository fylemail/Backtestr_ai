...
mod tests {
    ...
    fn create_test_tick(symbol: &str, offset_secs: i64) -> Tick {
        ...
        Tick::new(symbol.to_string(), timestamp, 1.0921, 1.0923)
    ...
    fn test_query_ticks_by_symbol_and_time() -> Result<()> {
        ...
        let now = Utc::now();
        let ticks = vec![
            Tick::new("EURUSD".to_string(), now - Duration::hours(2), 1.0920, 1.0922),
    ...
    fn test_delete_ticks_by_time_range() -> Result<()> {
        ...
        let deleted = db.delete_ticks_by_time_range(
